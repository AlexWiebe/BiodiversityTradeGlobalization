...
## Description: Code to create figures showing the effects of
## changing various biogeographic variables.
...
params = c(s1 = 1/20, s2 = 1/20, d1 = 0.001, d2 = 0.001, a1 = 1/20, a2 = 1/20, h = 1,
           r = 0.04, m1 = 0.005, m2 = 0.005, e = 0.5)
...
# # # # If testing the impact of a different c value:
# c1 = 50
# c2 = 150
# z2 = 0.25
# z1 = 0.25
...
full.ag = full.ag %>% add_column(dS_X1 = NA) %>%
  add_column(dS_X2 = NA)
...
for (i in 1:length(full.ag$time)){
  if (i == 1){
    ...
    dA11 = max(full.ag$A11[i] - full.ag$A11[i-1], 0)
    dA12 = max(full.ag$A12[i] - full.ag$A12[i-1], 0)
    dA21 = max(full.ag$A21[i] - full.ag$A21[i-1], 0)
    dA22 = max(full.ag$A22[i] - full.ag$A22[i-1], 0)
